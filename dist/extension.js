...
Line: `+p.line+`
Column: `+p.column+`
Char: `+p.c),o=new Error(o),p.error=o,D(p,"onerror",o),p}function lt(p){return p.sawRoot&&!p.closedRoot&&H(p,"Unclosed root tag"),p.state!==m.BEGIN&&p.state!==m.BEGIN_WHITESPACE&&p.state!==m.TEXT&&J(p,"Unexpected end"),F(p),p.c="",p.closed=!0,D(p,"onend"),i.call(p,p.strict,p.opt),p}function H(p,o){if(typeof p!="object"||!(p instanceof i))throw new Error("bad call to strictFail");p.strict&&J(p,o)}function Be(p){p.strict||(p.tagName=p.tagName[p.looseCase]());var o=p.tags[p.tags.length-1]||p,N=p.tag={name:p.tagName,attributes:{}};p.opt.xmlns&&(N.ns=o.ns),p.attribList.length=0,M(p,"onopentagstart",N)}function Vt(p,o){var N=p.indexOf(":"),_=N<0?["",p]:p.split(":"),V=_[0],G=_[1];return o&&p==="xmlns"&&(V="xmlns",G=""),{prefix:V,local:G}}function jt(p){if(p.strict||(p.attribName=p.attribName[p.looseCase]()),p.attribList.indexOf(p.attribName)!==-1||p.tag.attributes.hasOwnProperty(p.attribName)){p.attribName=p.attribValue="";return}if(p.opt.xmlns){var o=Vt(p.attribName,!0),N=o.prefix,_=o.local;if(N==="xmlns")if(_==="xml"&&p.attribValue!==h)H(p,"xml: prefix must be bound to "+h+`
Actual: `+p.attribValue);else if(_==="xmlns"&&p.attribValue!==I)H(p,"xmlns: prefix must be bound to "+I+`
Actual: `+p.attribValue);else{var V=p.tag,G=p.tags[p.tags.length-1]||p;V.ns===G.ns&&(V.ns=Object.create(G.ns)),V.ns[_]=p.attribValue}p.attribList.push([p.attribName,p.attribValue])}else p.tag.attributes[p.attribName]=p.attribValue,M(p,"onattribute",{name:p.attribName,value:p.attribValue});p.attribName=p.attribValue=""}function bt(p,o){if(p.opt.xmlns){var N=p.tag,_=Vt(p.tagName);N.prefix=_.prefix,N.local=_.local,N.uri=N.ns[_.prefix]||"",N.prefix&&!N.uri&&(H(p,"Unbound namespace prefix: "+JSON.stringify(p.tagName)),N.uri=_.prefix);var V=p.tags[p.tags.length-1]||p;N.ns&&V.ns!==N.ns&&Object.keys(N.ns).forEach(function(di){M(p,"onopennamespace",{prefix:di,uri:N.ns[di]})});for(var G=0,K=p.attribList.length;G<K;G++){var tt=p.attribList[G],nt=tt[0],W=tt[1],Q=Vt(nt,!0),pt=Q.prefix,vr=Q.local,li=pt===""?"":N.ns[pt]||"",Ue={name:nt,value:W,prefix:pt,local:vr,uri:li};pt&&pt!=="xmlns"&&!li&&(H(p,"Unbound namespace prefix: "+JSON.stringify(pt)),Ue.uri=pt),p.tag.attributes[nt]=Ue,M(p,"onattribute",Ue)}p.attribList.length=0}p.tag.isSelfClosing=!!o,p.sawRoot=!0,p.tags.push(p.tag),M(p,"onopentag",p.tag),o||(!p.noscript&&p.tagName.toLowerCase()==="script"?p.state=m.SCRIPT:p.state=m.TEXT,p.tag=null,p.tagName=""),p.attribName=p.attribValue="",p.attribList.length=0}function Ht(p){if(!p.tagName){H(p,"Weird empty close tag."),p.textNode+="</>",p.state=m.TEXT;return}if(p.script){if(p.tagName!=="script"){p.script+="</"+p.tagName+">",p.tagName="",p.state=m.SCRIPT;return}M(p,"onscript",p.script),p.script=""}var o=p.tags.length,N=p.tagName;p.strict||(N=N[p.looseCase]());for(var _=N;o--;){var V=p.tags[o];if(V.name!==_)H(p,"Unexpected close tag");else break}if(o<0){H(p,"Unmatched closing tag: "+p.tagName),p.textNode+="</"+p.tagName+">",p.state=m.TEXT;return}p.tagName=N;for(var G=p.tags.length;G-- >o;){var K=p.tag=p.tags.pop();p.tagName=p.tag.name,M(p,"onclosetag",p.tagName);var tt={};for(var nt in K.ns)tt[nt]=K.ns[nt];var W=p.tags[p.tags.length-1]||p;p.opt.xmlns&&K.ns!==W.ns&&Object.keys(K.ns).forEach(function(Q){var pt=K.ns[Q];M(p,"onclosenamespace",{prefix:Q,uri:pt})})}o===0&&(p.closedRoot=!0),p.tagName=p.attribValue=p.attribName="",p.attribList.length=0,p.state=m.TEXT}function $e(p){var o=p.entity,N=o.toLowerCase(),_,V="";return p.ENTITIES[o]?p.ENTITIES[o]:p.ENTITIES[N]?p.ENTITIES[N]:(o=N,o.charAt(0)==="#"&&(o.charAt(1)==="x"?(o=o.slice(2),_=parseInt(o,16),V=_.toString(16)):(o=o.slice(1),_=parseInt(o,10),V=_.toString(10))),o=o.replace(/^0+/,""),isNaN(_)||V.toLowerCase()!==o||_<0||_>1114111?(H(p,"Invalid character entity"),"&"+p.entity+";"):String.fromCodePoint(_))}function ne(p,o){o==="<"?(p.state=m.OPEN_WAKA,p.startTagPosition=p.position):r(o)||(H(p,"Non-whitespace before first tag."),p.textNode=o,p.state=m.TEXT)}function Xt(p,o){var N="";return o<p.length&&(N=p.charAt(o)),N}function re(p){var o=this;if(this.error)throw this.error;if(o.closed)return J(o,"Cannot write after close. Assign an onready handler.");if(p===null)return lt(o);typeof p=="object"&&(p=p.toString());for(var N=0,_="";_=Xt(p,N++),o.c=_,!!_;)switch(o.trackPosition&&(o.position++,_===`
...
const RULES = ${e};
const TARGET = ${pi(t.target_data)};
${Nr}
</script>
</body>
</html>`}function Ar(t){let e=[t.host_name&&`Host: ${t.host_name}`,t.ip_address&&`IP: ${t.ip_address}`,t.fqdn&&`FQDN: ${t.fqdn}`,t.mac_address&&`MAC: ${t.mac_address}`,t.target_type&&`Type: ${t.target_type}`,t.role&&t.role!=="None"&&`Role: ${t.role}`].filter(Boolean);return e.length?`<div class="target-row">${e.map(i=>`<span class="chip">${ft(i)}</span>`).join("")}<button class="btn-edit-target" id="editTargetBtn">Edit Target</button></div>`:'<p class="muted">No target data configured <button class="btn-edit-target" id="editTargetBtn">Edit Target</button></p>'}function Dr(t,e){let i=Er(t.rules),l=n=>i.total?Math.round(n/i.total*100):0;return`
<section class="stig" data-idx="${e}">
  ...
  <div class="tbl-wrap">
    <table class="tbl">
      ...
      <tbody>
        ${t.rules.map(n=>kr(n,t.uuid)).join(`
`)}
      </tbody>
    </table>
  </div>
</section>`}function kr(t,e){let i=`${t.group_id} ${t.rule_version} ${t.rule_title} ${t.group_title}`.toLowerCase();return`<tr class="row" data-uuid="${t.uuid}" data-stig="${e}"
  ...
  <td><span class="pill sev-${t.severity}">CAT ${Sr(t.severity)}</span></td>
...
`)}
        ...
        <SEVERITY_JUSTIFICATION></SEVERITY_JUSTIFICATION>
          ...
          </SI_DATA>`}function Fr(t){let e=[ht("version",t.version),ht("classification","UNCLASSIFIED"),ht("customname",""),ht("stigid",t.stig_id),ht("description",""),ht("filename",""),ht("releaseinfo",t.release_info),ht("title",t.stig_name),ht("uuid",t.uuid),ht("notice","terms-of-use"),ht("source","")].join(`
`),i=t.rules.map(Rr).join(`
`);return`    <iSTIG>
        <STIG_INFO>
${e}
        </STIG_INFO>
...
</body></html>`}function Ne(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}var Kr=`
*{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:var(--vscode-editor-background,#1e1e1e);
  --fg:var(--vscode-editor-foreground,#ccc);
...
.sort-asc::after{content:' \\25B2';font-size:.7em}
.sort-desc::after{content:' \\25BC';font-size:.7em}
`;var Bt=U(require("vscode")),Qe=U(require("fs")),jn={not_reviewed:"Not Reviewed",open:"Open",not_a_finding:"Not a Finding",not_applicable:"Not Applicable"},Hn={open:0,not_reviewed:1,not_applicable:2,not_a_finding:3};function Yr(t){return t==="high"?"I":t==="medium"?"II":"III"}var Pe=class{static async show(e){let i=e;if(!i){let x=await Bt.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Select FIRST checklist (baseline / old)"});if(!x?.[0])return;i=x[0]}let l=await Bt.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Select SECOND checklist (current / new)"});if(!l?.[0])return;let n=JSON.parse(Qe.readFileSync(i.fsPath,"utf-8")),s=JSON.parse(Qe.readFileSync(l[0].fsPath,"utf-8")),c=new Map;for(let x of n.stigs)for(let v of x.rules)c.set(v.rule_version,v);let u=new Map;for(let x of s.stigs)for(let v of x.rules)u.set(v.rule_version,v);let d=[],f=new Set([...c.keys(),...u.keys()]);for(let x of f){let v=c.get(x),r=u.get(x);if(v&&r){let a="unchanged";v.status!==r.status&&(a=Hn[r.status]>Hn[v.status]?"improved":"regression"),d.push({ruleVersion:x,groupId:r.group_id,ruleTitle:r.rule_title,severity:r.severity,statusA:v.status,statusB:r.status,change:a})}else r&&!v?d.push({ruleVersion:x,groupId:r.group_id,ruleTitle:r.rule_title,severity:r.severity,statusA:"",statusB:r.status,change:"new"}):v&&!r&&d.push({ruleVersion:x,groupId:v.group_id,ruleTitle:v.rule_title,severity:v.severity,statusA:v.status,statusB:"",change:"removed"})}let g={regression:0,new:1,improved:2,removed:3,unchanged:4};d.sort((x,v)=>g[x.change]-g[v.change]);let y=d.filter(x=>x.change==="regression").length,w=d.filter(x=>x.change==="improved").length,h=d.filter(x=>x.change==="new").length,I=d.filter(x=>x.change==="removed").length,T=d.filter(x=>x.change==="unchanged").length,E=d.filter(x=>x.change!=="unchanged"),k=Bt.window.createWebviewPanel("stigViewer.diff","STIG Diff",Bt.ViewColumn.One,{enableScripts:!0});k.webview.html=Jr(i.fsPath,l[0].fsPath,E,d,{regressions:y,improvements:w,newRules:h,removed:I,unchanged:T})}};function Jt(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Jr(t,e,i,l,n){let s=t.split("/").pop()||t,c=e.split("/").pop()||e,u={regression:"\u2193 Regression",improved:"\u2191 Improved",new:"+ New",removed:"\u2212 Removed",unchanged:"= Unchanged"},d={regression:"chg-reg",improved:"chg-imp",new:"chg-new",removed:"chg-rem",unchanged:"chg-unc"},f=g=>g.map(y=>`<tr class="row ${d[y.change]}" data-change="${y.change}">
    <td><span class="pill sev-${y.severity}">CAT ${Yr(y.severity)}</span></td>
    <td><code>${Jt(y.groupId)}</code></td>
    <td><code>${Jt(y.ruleVersion)}</code></td>
    <td class="td-title">${Jt(y.ruleTitle)}</td>
    <td>${y.statusA?`<span class="pill st-${y.statusA}">${jn[y.statusA]||y.statusA}</span>`:'<span class="muted">\u2014</span>'}</td>
    <td>${y.statusB?`<span class="pill st-${y.statusB}">${jn[y.statusB]||y.statusB}</span>`:'<span class="muted">\u2014</span>'}</td>
...
`;var $t=U(require("vscode")),Qt=U(require("fs")),Le=U(Kt());function Xn(t){return t==null?[]:Array.isArray(t)?t:[t]}function Me(t){return t==null?"":typeof t=="string"?t:Array.isArray(t)?Me(t[0]):typeof t=="object"&&"_"in t?String(t._):String(t)}var Zr={pass:"not_a_finding",fail:"open",error:"open",unknown:"not_reviewed",notapplicable:"not_applicable",notchecked:"not_reviewed",informational:"not_reviewed",fixed:"not_a_finding"};function to(t){let e=t.match(/(SV-\d+r\d+_rule)/);return e?e[1]:t}async function Gn(t){let e=t;if(!e){let y=await $t.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Select checklist to update with SCAP results"});if(!y?.[0])return;e=y[0]}let i=await $t.window.showOpenDialog({canSelectMany:!1,filters:{"XCCDF Results":["xml"]},title:"Select SCAP XCCDF results file"});if(!i?.[0])return;let l=JSON.parse(Qt.readFileSync(e.fsPath,"utf-8")),n=Qt.readFileSync(i[0].fsPath,"utf-8"),s=await(0,Le.parseStringPromise)(n,{explicitArray:!1,tagNameProcessors:[Le.processors.stripPrefix]}),c=null;if(s.TestResult?c=s.TestResult:s.Benchmark?.TestResult&&(c=s.Benchmark.TestResult,Array.isArray(c)&&(c=c[c.length-1])),!c)throw new Error("No <TestResult> element found in the XCCDF results file.");let u=new Map;for(let y of Xn(c["rule-result"])){let w=y?.$?.idref||"",h=to(w),I=Me(y.result).toLowerCase(),T=Zr[I]||"not_reviewed",E="",k=Xn(y.check)[0];k&&(E=Me(k["check-content"])),!E&&y.message&&(E=Me(y.message)),u.set(h,{status:T,detail:E})}let d=0,f=0,g=new Date().toISOString();for(let y of l.stigs)for(let w of y.rules){let h=u.get(w.rule_id);h?(w.status=h.status,h.detail&&(w.finding_details=h.detail),w.updatedAt=g,d++):f++}Qt.writeFileSync(e.fsPath,JSON.stringify(l,null,2)),await $t.commands.executeCommand("vscode.openWith",e,"stigViewer.cklbEditor"),$t.window.showInformationMessage(`SCAP import: ${d} rules updated, ${f} unmatched`)}var yt=U(require("vscode")),wt=U(require("fs")),kt=U(require("path"));function eo(t){let e=kt.join(__dirname,"..","scan-patterns.json"),i=t||e;if(!wt.existsSync(i))throw new Error(`Scan patterns file not found: ${i}`);return JSON.parse(wt.readFileSync(i,"utf-8"))}function io(t,e){let i=[e.rule_title,e.discussion,e.check_content,e.group_title].join(" ").toLowerCase();return t.ruleKeywords.some(l=>i.includes(l.toLowerCase()))}function Wn(t){let e=t.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*\*/g,"__DOUBLESTAR__").replace(/\*/g,"[^/]*").replace(/__DOUBLESTAR__/g,".*").replace(/\?/g,".");return new RegExp(`^${e}$`)}function Ze(t,e){let i=t.replace(/\\/g,"/");return e.some(l=>{if(l.includes("{")){let n=l.match(/^(.*)\{([^}]+)\}(.*)$/);if(n){let[,s,c,u]=n;return c.split(",").some(d=>{let f=s+d.trim()+u;return Wn(f).test(i)})}}return Wn(l).test(i)})}function no(t,e){let i=[];function l(n){let s;try{s=wt.readdirSync(n,{withFileTypes:!0})}catch{return}for(let c of s){let u=kt.join(n,c.name),d=kt.relative(t,u);Ze(d,e)||(c.isDirectory()?Ze(d+"/",e)||l(u):c.isFile()&&i.push(u))}}return l(t),i}function ro(t,e,i){let l;try{let c=wt.readFileSync(t);if(c.includes(0))return[];l=c.toString("utf-8")}catch{return[]}let n=l.split(`
`),s=[];for(let c of e){let u=i?"g":"gi",d;try{d=new RegExp(c,u)}catch{continue}for(let f=0;f<n.length;f++)d.test(n[f])&&s.push({line:f+1,content:n[f].trim().substring(0,200)}),d.lastIndex=0}return s}async function Kn(t){let e=t;if(!e){let d=await yt.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Select checklist to populate with scan results"});if(!d?.[0])return;e=d[0]}let i=await yt.window.showOpenDialog({canSelectFolders:!0,canSelectFiles:!1,canSelectMany:!1,title:"Select repository folder to scan"});if(!i?.[0])return;let l=await yt.window.showQuickPick(["Use default scan patterns","Select custom scan-patterns.json"],{title:"Scan pattern configuration"});if(!l)return;let n;if(l.includes("custom")){let d=await yt.window.showOpenDialog({canSelectMany:!1,filters:{JSON:["json"]},title:"Select scan-patterns.json"});if(!d?.[0])return;n=d[0].fsPath}let s=eo(n),c=JSON.parse(wt.readFileSync(e.fsPath,"utf-8")),u=i[0].fsPath;await yt.window.withProgress({location:yt.ProgressLocation.Notification,title:"STIG Repo Scan",cancellable:!0},async(d,f)=>{d.report({message:"Discovering files..."});let g=no(u,s.excludeGlobs);if(f.isCancellationRequested)return;let y=[],w=new Date().toISOString(),h=0,I=0,T=0,E=0,k=c.stigs.reduce((v,r)=>v+r.rules.length,0);for(let v of c.stigs)for(let r of v.rules){if(f.isCancellationRequested)return;let a=s.scanPatterns.filter(P=>io(P,r));if(a.length===0)continue;h++,d.report({message:`Scanning ${r.group_id} (${h}/${k})`,increment:1/k*100});let b=[],A=[];for(let P of a){let R=g.filter(M=>{let F=kt.relative(u,M);return Ze(F,P.fileGlobs)}),L=!1;for(let M of R){if(f.isCancellationRequested)return;let F=ro(M,P.patterns,P.caseSensitive);if(F.length>0){L=!0;let $=kt.relative(u,M);for(let J of F)b.push({file:$,line:J.line,content:J.content,patternId:P.id})}}L&&A.push(P.name)}let S=a.some(P=>P.matchMeansOpen),m=a.filter(P=>P.matchMeansOpen),C=b.filter(P=>m.some(R=>R.id===P.patternId)),D,O;if(C.length>0){D="open",T++;let P=a.find(R=>R.findingDetailOverride&&b.some(L=>L.patternId===R.id));if(P?.findingDetailOverride){let R=b.filter(L=>L.patternId===P.id).slice(0,20).map(L=>`  ${L.file}:${L.line} \u2014 ${L.content}`).join(`
...
`)}var Fe=class t{static async show(e){let i=e;if(!i){let g=await rt.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Select STIG Checklist for Crosswalk"});if(!g?.[0])return;i=g[0]}let l=JSON.parse(zt.readFileSync(i.fsPath,"utf-8")),n=await rt.window.showOpenDialog({canSelectMany:!1,filters:{CSV:["csv"]},title:"Select NIST 800-53 Controls CSV"});if(!n?.[0])return;let s=zt.readFileSync(n[0].fsPath,"utf-8"),c=sr(s);if(c.length===0){rt.window.showErrorMessage("No valid NIST 800-53 control IDs found in the CSV. Expected values like AC-2, CM-7, SI-10.");return}let u;if((await rt.window.showQuickPick(["Use bundled CCI mapping (covers common CCIs)","Import full CCI List XML from DISA"],{title:"CCI Mapping Source"}))?.includes("Import")){let g=await rt.window.showOpenDialog({canSelectMany:!1,filters:{"CCI List (XML)":["xml","XML"],"All Files":["*"]},title:"Select U_CCI_List.xml (extract U_CCI_List.zip from cyber.mil first)"});if(g?.[0])try{let y=zt.readFileSync(g[0].fsPath,"utf-8");u=await rr(y),rt.window.showInformationMessage(`Loaded ${Object.keys(u).length} CCI entries from XML.`)}catch(y){rt.window.showWarningMessage(`Failed to parse CCI List XML: ${y}. Falling back to bundled map.`)}}let f=ar(l,c,u);if(f.unmappedCciCount>0&&rt.window.showInformationMessage(`${f.unmappedCciCount} CCIs in this checklist could not be mapped. Import the full CCI List XML for complete coverage.`),t._panel)t._panel.reveal();else{let g=rt.window.createWebviewPanel("stigViewer.crosswalk",`NIST 800-53 Crosswalk \u2014 ${l.title}`,rt.ViewColumn.One,{enableScripts:!0,retainContextWhenHidden:!0});t._panel=g,g.onDidDispose(()=>{t._panel=void 0}),g.webview.onDidReceiveMessage(async y=>{if(y.type==="exportCsv"){let w=Nt.basename(i.fsPath,".cklb")+"_NIST_crosswalk.csv",h=await rt.window.showSaveDialog({defaultUri:rt.Uri.file(Nt.join(Nt.dirname(i.fsPath),w)),filters:{CSV:["csv"]},title:"Export Crosswalk Report"});h&&(zt.writeFileSync(h.fsPath,cr(f),"utf-8"),rt.window.showInformationMessage(`Exported crosswalk \u2192 ${Nt.basename(h.fsPath)}`))}})}t._panel.webview.html=mo(f)}};function st(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function fo(t){switch(t){case"fully_satisfied":return'<span class="st-icon st-sat">\u2714</span>';case"partially_satisfied":return'<span class="st-icon st-par">\u25D1</span>';case"at_risk":return'<span class="st-icon st-risk">\u26A0</span>';default:return'<span class="st-icon st-gap">\u25CF</span>'}}function ho(t){return t.replace(/_/g," ")}function mo(t){let i=[...new Set(t.controlMappings.map(s=>s.control.family).filter(Boolean))].sort().map(s=>`<option value="${st(s)}">${st(s)}</option>`).join(""),l=t.totalNistControls?Math.round(t.coveredControls/t.totalNistControls*100):0,n=t.controlMappings.map((s,c)=>{let u=s.mappedRules.filter(h=>h.mappedStatus==="passing").length,d=s.mappedRules.filter(h=>h.mappedStatus==="open").length,f=s.mappedRules.filter(h=>h.mappedStatus==="not_reviewed").length,g=s.mappedRules.map(h=>{let I=h.severity==="high"?"sev-high":h.severity==="medium"?"sev-med":"sev-low",T=h.mappedStatus==="passing"?"rs-pass":h.mappedStatus==="open"?"rs-open":"rs-nr";return`<tr>
        ...
        <td class="sub-td ${T}">${st(ho(h.status))}</td>
...
.sort-asc::after{content:' \\25B2';font-size:.7em}
.sort-desc::after{content:' \\25BC';font-size:.7em}
`;var z=U(require("vscode")),ai=U(require("path"));var Ot="The selected file is not valid HDF JSON. Expected structure: profiles[] with controls[].";function ur(t){let e;try{e=JSON.parse(t)}catch(l){throw new Error(`${Ot} (JSON parse error: ${l.message})`)}if(!e||typeof e!="object")throw new Error(Ot);let i=e;if(!Array.isArray(i.profiles)||i.profiles.length===0)throw new Error(Ot);for(let l of i.profiles){if(!l||typeof l!="object")throw new Error(Ot);if(!Array.isArray(l.controls))throw new Error(Ot);for(let n of l.controls){if(!n||typeof n!="object")throw new Error(Ot);if(typeof n.id!="string"||typeof n.title!="string")throw new Error(Ot);(!n.tags||typeof n.tags!="object")&&(n.tags={})}}return i}function pr(t){if(!t||t.length===0)return"not_reviewed";if(t.some(s=>s.status==="failed"))return"open";let i=t.some(s=>s.status==="error"),l=t.some(s=>s.status==="passed");return t.every(s=>s.status==="skipped")?"not_applicable":i&&!l?"not_reviewed":l?"not_a_finding":"not_reviewed"}function yo(t){let e=t.tags?.severity;return e==="high"||e==="medium"||e==="low"?e:typeof t.impact=="number"?t.impact>=.7?"high":t.impact>=.4?"medium":"low":"medium"}function oi(t,e){return t.descriptions?t.descriptions.find(l=>l.label===e)?.data??"":""}function vo(t){return t.desc?t.desc:oi(t,"default")}function fr(t){if(!t.results||t.results.length===0)return t.code?`[HDF IMPORT] No test results recorded.
...
</html>`}function Do(t){t.subscriptions.push(at.register(t)),t.subscriptions.push(j.commands.registerCommand("stigViewer.openFile",async()=>{let e=await j.window.showOpenDialog({canSelectMany:!1,filters:{"STIG Checklist":["cklb"]},title:"Open STIG Checklist (.cklb)"});e?.[0]&&await j.commands.executeCommand("vscode.openWith",e[0],"stigViewer.cklbEditor")})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importXccdf",async e=>{if(!e){let i=await j.window.showOpenDialog({canSelectMany:!1,filters:{"XCCDF Benchmark":["xml"]},title:"Import XCCDF Benchmark"});if(!i?.[0])return;e=i[0]}try{await Un(e)}catch(i){j.window.showErrorMessage(`XCCDF import failed: ${i}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importCkl",async e=>{if(!e){let i=await j.window.showOpenDialog({canSelectMany:!1,filters:{"CKL Checklist":["ckl"]},title:"Import CKL Checklist"});if(!i?.[0])return;e=i[0]}try{await qn(e)}catch(i){j.window.showErrorMessage(`CKL import failed: ${i}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importScapResults",async()=>{try{await Gn(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`SCAP import failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.mergeFindings",async()=>{try{await zn(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`Merge failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.dashboard",async()=>{try{await Oe.show(t)}catch(e){j.window.showErrorMessage(`Dashboard failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.diffChecklists",async()=>{try{await Pe.show(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`Diff failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.scanRepo",async()=>{try{await Kn(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`Repo scan failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importSarif",async()=>{try{await Yn(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`SARIF import failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importAudit",async()=>{try{await Qn(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`Dependency audit import failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.exportEvidence",async()=>{try{await tr(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`Evidence package failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.nistCrosswalk",async()=>{try{await Fe.show(at.activeDocumentUri)}catch(e){j.window.showErrorMessage(`NIST crosswalk failed: ${e}`)}})),t.subscriptions.push(j.commands.registerCommand("stigViewer.importHdf",async e=>{try{await yr(t,e)}catch(i){j.window.showErrorMessage(`HDF import failed: ${i}`)}}))}function ko(){}0&&(module.exports={activate,deactivate});
