...
// src/hdfImportPanel.ts
var vscode13 = __toESM(require("vscode"));
var path7 = __toESM(require("path"));

// src/hdfImporter.ts
var INVALID_HDF_MSG = "The selected file is not valid HDF JSON. Expected structure: profiles[] with controls[].";
function parseHdf(jsonContent) {
  let obj;
  try {
    obj = JSON.parse(jsonContent);
  } catch (e) {
    throw new Error(`${INVALID_HDF_MSG} (JSON parse error: ${e.message})`);
  }
...
function deriveHdfSeverity(control) {
...
${snippet}`;
  ...
  const lines = ["[HDF IMPORT] InSpec automated scan results:", ""];
  for (let i = 0; i < control.results.length; i++) {
    ...
    if (result.run_time !== void 0) lines.push(`  Duration: ${result.run_time}s`);
    if (result.start_time) lines.push(`  Started: ${result.start_time}`);
    lines.push("");
  }
  return lines.join("\n").trim();
}
function mergeFindingDetails(existing, incoming, preserve) {
...
function updateTargetDataFields(td, hdf, touchedProfiles) {
...
${snippet}`;
  ...
  if (touchedProfiles.length > 1) {
    const noteLine = `[HDF] Scan covered ${touchedProfiles.length} profiles: ${touchedProfiles.map((p) => p.title || p.name).join(", ")}`;
    if (!td.comments) {
...
function applyHdfToChecklist(doc, hdf, options) {
  const opts = {
  ...
  const byRuleVersion = /* @__PURE__ */ new Map();
  ...
  const matchedRuleUuids = /* @__PURE__ */ new Set();
  let totalControls = 0;
  let missingStigId = 0;
  ...
  for (const { control, rule, matchedBy } of matchedByKey.values()) {
    ...
    const canOverwriteStatus = opts.overwriteExistingStatus || rule.status === "not_reviewed";
...
function controlToRule(control, stigUuid, now) {
  ...
  const ccis = Array.isArray(control.tags?.cci) ? [...control.tags.cci] : [];
  const legacyIds = Array.isArray(control.tags?.legacy) ? [...control.tags.legacy] : [];
  return {
    uuid: ruleUuid,
    stig_uuid: stigUuid,
    target_key: null,
...
function hdfToNewChecklist(hdf) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const docId = v4_default();
  const stigs = [];
  for (const profile of hdf.profiles) {
    const stigUuid = v4_default();
    const rules = profile.controls.map((c) => controlToRule(c, stigUuid, now));
  ...
  const firstProfile = hdf.profiles[0];
  const title = firstProfile?.title || firstProfile?.name || "InSpec / HDF Import";
  return {
    title,
    id: docId,
    stigs,
...
function generateHdfImportReport(summary, checklistTitle) {
  const lines = [];
  ...
  lines.push(`**Imported:** ${(/* @__PURE__ */ new Date()).toISOString()}`);
  ...
  lines.push(
    `| Not Reviewed (error/no results) | ${summary.statusBreakdown.error + summary.statusBreakdown.noResults} |`
  );
  lines.push("");
  ...
  if (summary.appliedChanges.length === 0) {
    ...
    for (const ch of summary.appliedChanges) {
      lines.push(
        `| ${mdEscape(ch.ruleVersion)} | ${mdEscape(ch.groupId)} | ${STATUS_LABELS3[ch.oldStatus] || ch.oldStatus} | ${STATUS_LABELS3[ch.newStatus] || ch.newStatus} | ${ch.matchedBy} |`
      );
    }
  }
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "These controls in the HDF file had no corresponding rule in the checklist. This may indicate a STIG version mismatch or that the HDF covers additional controls beyond the checklist's STIG."
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    "These rules in the checklist were not addressed by any HDF control. They retain their previous status."
  );
  lines.push("");
...
// src/hdfImportPanel.ts
async function importHdfFlow(context, uri) {
  let hdfUri = uri;
  if (!hdfUri) {
    const picks = await vscode13.window.showOpenDialog({
      canSelectMany: false,
  ...
  let hdf;
  try {
    const bytes = await vscode13.workspace.fs.readFile(hdfUri);
    const content = new TextDecoder("utf-8").decode(bytes);
    hdf = parseHdf(content);
  } catch (e) {
    vscode13.window.showErrorMessage(e.message);
    return;
  }
      ...
      [
        {
          ...
          description: path7.basename(activeUri.fsPath),
          value: "apply"
        },
        {
  ...
  let checklistUri = null;
  if (mode === "apply" && activeUri) {
    try {
      const doc = await vscode13.workspace.openTextDocument(activeUri);
      checklistDoc = JSON.parse(doc.getText());
      checklistUri = activeUri;
    } catch (e) {
      vscode13.window.showErrorMessage(`Failed to read active checklist: ${e}`);
      return;
    }
  }
  openImportPanel(context, { hdf, checklistDoc, checklistUri, mode });
}
function openImportPanel(context, state) {
  ...
  panel.webview.html = buildWebviewHtml2();
  const sendInitial = () => {
  ...
  const readySub = panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    if (msg?.type === "downloadReport" && msg.summary && msg.checklistTitle !== void 0) {
      await downloadReport(msg.summary, msg.checklistTitle);
    }
    if (msg?.type === "cancel") {
      panel.dispose();
    }
...
function projectApply(doc, hdf, options) {
  const { summary } = applyHdfToChecklist(doc, hdf, options);
  return { summary };
}
async function runModeA(panel, state, options) {
  ...
  try {
    ...
    const document = await vscode13.workspace.openTextDocument(state.checklistUri);
    const edit = new vscode13.WorkspaceEdit();
    const fullRange = new vscode13.Range(
      document.positionAt(0),
      document.positionAt(document.getText().length)
    );
    edit.replace(document.uri, fullRange, JSON.stringify(updated, null, 2));
    const applied = await vscode13.workspace.applyEdit(edit);
    if (!applied) {
      vscode13.window.showErrorMessage("HDF import: failed to apply edit to the checklist.");
      return;
    }
    await document.save();
    ...
    vscode13.window.showErrorMessage(`HDF import failed: ${e}`);
  }
}
async function runModeB(panel, state) {
  try {
    const newDoc = hdfToNewChecklist(state.hdf);
    const defaultName = (newDoc.title || "hdf-import").replace(/\s+/g, "-") + ".cklb";
    const saveUri = await vscode13.window.showSaveDialog({
      title: "Save New Checklist",
      defaultUri: vscode13.Uri.file(defaultName),
    ...
    await vscode13.workspace.fs.writeFile(saveUri, bytes);
    await vscode13.commands.executeCommand("vscode.openWith", saveUri, "stigViewer.cklbEditor");
    ...
    vscode13.window.showErrorMessage(`HDF new-checklist import failed: ${e}`);
    panel.dispose();
  }
}
async function downloadReport(summary, checklistTitle) {
  ...
  await vscode13.workspace.fs.writeFile(saveUri, bytes);
  vscode13.window.showInformationMessage(`HDF report saved: ${path7.basename(saveUri.fsPath)}`);
}
function getNonce() {
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.openFile", async () => {
      ...
      if (uris?.[0]) {
        await vscode14.commands.executeCommand("vscode.openWith", uris[0], "stigViewer.cklbEditor");
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importXccdf", async (uri) => {
      if (!uri) {
        const uris = await vscode14.window.showOpenDialog({
          canSelectMany: false,
      ...
      try {
        ...
        vscode14.window.showErrorMessage(`XCCDF import failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importCkl", async (uri) => {
      if (!uri) {
        const uris = await vscode14.window.showOpenDialog({
          canSelectMany: false,
          filters: { "CKL Checklist": ["ckl"] },
      ...
      try {
        ...
        vscode14.window.showErrorMessage(`CKL import failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importScapResults", async () => {
      try {
        await importScapResults(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`SCAP import failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.mergeFindings", async () => {
      try {
        await mergeFindings(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`Merge failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.dashboard", async () => {
      try {
        await DashboardPanel.show(context);
      } catch (e) {
        vscode14.window.showErrorMessage(`Dashboard failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.diffChecklists", async () => {
      try {
        await DiffPanel.show(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`Diff failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.scanRepo", async () => {
      try {
        await scanRepo(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`Repo scan failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importSarif", async () => {
      try {
        await importSarif(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`SARIF import failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importAudit", async () => {
      try {
        await importAudit(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`Dependency audit import failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.exportEvidence", async () => {
      try {
        await exportEvidence(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`Evidence package failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.nistCrosswalk", async () => {
      try {
        await CrosswalkPanel.show(CklbEditorProvider.activeDocumentUri);
      } catch (e) {
        vscode14.window.showErrorMessage(`NIST crosswalk failed: ${e}`);
      }
    })
  );
  context.subscriptions.push(
    vscode14.commands.registerCommand("stigViewer.importHdf", async (uri) => {
      try {
        await importHdfFlow(context, uri);
      } catch (e) {
        vscode14.window.showErrorMessage(`HDF import failed: ${e}`);
      }
    })
  );
}
